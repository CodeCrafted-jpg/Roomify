...
import { ArrowBigDown, ArrowRight, ArrowUpRight, Clock, Layers } from "lucide-react";
...
export default function Home() {
    ...
    useEffect(() => {
      ...
      // Deduplicate by ID and ensure all have IDs
      const uniqueProjects = Array.from(
        new Map(fetchedProjects.filter(p => p.id).map(p => [p.id, p])).values()
      );
    ...
    const handleDelete = async (e: React.MouseEvent, id: string) => {
        ...
        if (!ok) {
            ...
            const uniqueProjects = Array.from(new Map(fetched.map(p => [p.id, p])).values());
            setProjects(uniqueProjects);
        }
    };
    return (
