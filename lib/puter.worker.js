...
router.post('/api/projects/delete', async ({ request, user }) => {
  try {
    ...
    try {
      ...
      return jsonError(400, 'Invalid request body', { parseError: parseError.message });
    }

    const id = body?.id;
    if (!id) {
      console.error('Project ID is missing. Body:', body);
      return jsonError(400, 'Project ID is required', { receivedBody: body });
    }
